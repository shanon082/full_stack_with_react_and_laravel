...
const Register = () => {
  ...
  const Submit = (ev) => {
    ...
    axiosClient.post('/register', payload).then(({ data }) => { // Destructure response
      const { user, token } = data; // Explicitly extract user and token
      setUser(user);
      setToken(token);
    }).catch((err) => {
      const { response } = err;
    ...
    <div className="flex flex-col w-full min-h-screen justify-center items-center bg-gradient-to-br from-blue-900 to-black">
      <div className="p-5 flex flex-col sm:w-full md:w-1/2 lg:w-1/3 rounded-md bg-white">
        ...
        <form onSubmit={Submit} className="flex flex-col">
          ...
          <label htmlFor="Password" className="text-black text-2xl mb-2">Password:</label> {/* Fixed label casing */}
